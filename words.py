...
def add_answer(user_id, word_id, result):
    sql = " ".join(("INSERT INTO answers (user_id, word_id, result)",
...
def get_word(word_id):
    sql = " ".join(("SELECT id, finnish_word, swedish_word, image_data",
...
def get_random_swedish_words(excluded_word, word_count):
    sql = " ".join(("SELECT swedish_word FROM words",
                    "WHERE swedish_word != :excluded_word",
                    "ORDER BY RANDOM() LIMIT :word_count"))
